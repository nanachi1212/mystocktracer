...
export function BreadthView({ data }: { data: Breadth }) {
	...
	].map(([label, value]) => <article key={String(label)}><span>{label}</span><strong>{value == null ? '—' : Number(value).toLocaleString('zh-TW')}</strong></article>)}</section>
	<section className="taiwan-detail-grid"><article><Activity /><span>上漲家數比率</span><strong>{formatTaiwanRatio(data?.advance_ratio)}</strong></article><article><BarChart3 /><span>上漲成交占比</span><strong>{formatTaiwanRatio(data?.advancing_amount_ratio)}</strong></article><article><span>總成交額</span><strong>{formatTaiwanTWD(data?.total_amount_twd)}</strong><small>缺少成交額 {data?.missing_amount_count == null ? '—' : data.missing_amount_count.toLocaleString('zh-TW')} 筆</small></article></section></>;
}

export function EmotionView({ data }: { data: Emotion }) {
	return <><section className="taiwan-emotion-summary"><div><span>市場狀態</span><strong>{taiwanStatusLabel(data?.state)}</strong></div><div><span>資料信心</span><strong>{taiwanStatusLabel(data?.confidence)}</strong></div><small>模型 {data?.model_version || '未提供'}</small></section><section className="taiwan-detail-grid"><article><span>市場參與</span><strong>{taiwanStatusLabel(data?.components?.breadth_participation)}</strong><small>上漲家數比率 {formatTaiwanRatio(data?.raw?.advance_ratio)}</small></article><article><span>成交方向</span><strong>{taiwanStatusLabel(data?.components?.capital_participation)}</strong><small>上漲成交占比 {formatTaiwanRatio(data?.raw?.advancing_amount_ratio)}</small></article><article><span>家數與成交關係</span><strong>{taiwanStatusLabel(data?.components?.breadth_capital_relationship)}</strong><small>方向涵蓋 {formatTaiwanRatio(data?.coverage?.direction_coverage)} · 成交涵蓋 {formatTaiwanRatio(data?.coverage?.amount_coverage)}</small></article></section></>;
}

export function IndustryView({ data }: { data: IndustryScope }) {
	return <><section className="taiwan-detail-grid"><article><Building2 /><span>官方分類涵蓋</span><strong>{formatTaiwanRatio(data?.industry_coverage)}</strong><small>{data?.classified_count == null || data?.eligible_universe_count == null ? '未提供' : `${data.classified_count.toLocaleString('zh-TW')} / ${data.eligible_universe_count.toLocaleString('zh-TW')} 檔`}</small></article><article><span>分類狀態</span><strong>{taiwanStatusLabel(data?.taxonomy_status)}</strong></article><article><span>市場快照</span><strong>{taiwanStatusLabel(data?.snapshot_status)}</strong></article></section><div className="taiwan-industry-list">{(data?.industries || []).map((item) => <article key={item.industry_id}><header><strong>{item.industry_name}</strong><span>{item.exchange} · {item.constituent_count} 檔</span></header><div><span>相對市場廣度 <b>{formatTaiwanPercent(item.relative_breadth == null ? null : item.relative_breadth * 100, true)}</b></span><span>相對成交方向 <b>{formatTaiwanPercent(item.relative_capital == null ? null : item.relative_capital * 100, true)}</b></span></div><small>方向涵蓋 {formatTaiwanRatio(item.data_quality?.direction_coverage)} · 成交涵蓋 {formatTaiwanRatio(item.data_quality?.amount_coverage)}</small></article>)}</div></>;
}

...
export function taiwanOverviewChangeTone(value?: number | null): 'up' | 'down' | 'flat' {
	return value == null ? 'flat' : value > 0 ? 'up' : value < 0 ? 'down' : 'flat';
}
...
export function TaiwanMarketView({ config, refreshKey, onNavigate }: { config: BackendConfig | null; refreshKey: number; onNavigate?: (target: 'taiwan-screener' | 'taiwan-breadth' | 'taiwan-emotion' | 'taiwan-industry') => void }) {
	...
	return <div className="market-data-view taiwan-market-view">
		...
		{selected && quote && <section className="market-index-detail"><header><div><span>{selected.exchange} · {selected.security_type.toUpperCase()} · {selected.currency}</span><h3>{selected.name} {selected.code}</h3><small>{quote.meta.is_realtime ? '即時行情' : '官方收盤資料'} · {quote.meta.trade_date} · {quote.meta.stale ? '資料較舊' : '最新完成交易資料'}</small></div><div><strong>{quote.price.toLocaleString('zh-TW')}</strong><em className={quote.change_percent > 0 ? 'up' : quote.change_percent < 0 ? 'down' : 'flat'}>{quote.change_percent > 0 ? '+' : ''}{quote.change_percent.toFixed(2)}%</em></div></header><SourceNotice meta={quote.meta} locale="zh-TW" /><div className="market-kline-table"><header><span>日期</span><span>開盤</span><span>最高</span><span>最低</span><span>收盤</span><span>漲跌</span></header>{lines.slice(-10).reverse().map((line) => <article key={line.time}><span>{new Date(line.time).toLocaleDateString('zh-TW')}</span><span>{line.open}</span><span>{line.high}</span><span>{line.low}</span><strong>{line.close}</strong><em className={taiwanOverviewChangeTone(line.change_percent)}>{formatTaiwanPercent(line.change_percent)}</em></article>)}</div></section>}

...
func (fixedTaiwanChip) Institutional(_ context.Context, security foundation.SecurityIdentity, _ int) (foundation.InstitutionalHistory,error){return foundation.InstitutionalHistory{Security:security,Data:[]foundation.InstitutionalFlow{{Canonical:security.Canonical}}},nil}
func (fixedTaiwanChip) Margin(_ context.Context, security foundation.SecurityIdentity, _ int) (foundation.MarginHistory,error){return foundation.MarginHistory{Security:security,Data:[]foundation.MarginTrading{{Canonical:security.Canonical}}},nil}

func TestTaiwanMarketRoutesResolveCanonicalIdentity(t *testing.T) {
	server := NewServer(Config{TaiwanDirectory: fixedTaiwanDirectory{}, TaiwanMarket: fixedTaiwanMarket{}, TaiwanChip: fixedTaiwanChip{}})
	for _, path := range []string{"/api/v1/tw/quotes?symbols=2330", "/api/v1/tw/kline?symbol=台積電&limit=5", "/api/v1/tw/indexes", "/api/v1/tw/institutional?symbol=2330&limit=20", "/api/v1/tw/margin?symbol=台積電&limit=20"} {

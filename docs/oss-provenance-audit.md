...
runtime、test、config、一般 docs、packaging tool 的 inherited／unclear 替換阻擋項均為零。`blockingFiles=14` 仍是對「把全部 repository 內容一律改授權」的檢視項，並非本次限定範圍 MIT 決策的技術 blocker；它們沒有被重新標成 original 或改採 MIT。新增法律文件路徑只調整 area，仍以相同 blob／來源與 SHA 規則判定 inherited。
...
- `npm.cmd run test:tooling`：23 tests 通過；包含歷史授權移存後仍為 inherited、仍需保留原權利，以及 executable 不因放進 LICENSES 目錄而豁免的回歸案例。

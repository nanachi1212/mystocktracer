...
export function TaiwanScreenerWorkspace({ config, refreshKey, onOpenResearch }: { config: BackendConfig | null; refreshKey: number; onOpenResearch: (canonical: string, context?: TaiwanResearchEntryContext | null) => void }) {
	...
	// M7G — deliberately its own flag, independent of showFinancials: the backend keeps
		...
		<div className="taiwan-screener-advanced-group">
			...
			{fundamentalsExpanded && <>
				...
				<div className="taiwan-screener-filter-grid taiwan-screener-advanced-grid">
					...
					<label><span>負債比率最小（%）</span><input type="number" inputMode="decimal" value={draft.minDebtRatio} onChange={(event) => set('minDebtRatio', event.target.value)} placeholder="不限" /></label>
					<label><span>負債比率最大（%）</span><input type="number" inputMode="decimal" value={draft.maxDebtRatio} onChange={(event) => set('maxDebtRatio', event.target.value)} placeholder="不限" /></label>
					<label><span>負債權益比最小（%）</span><input type="number" inputMode="decimal" value={draft.minDebtToEquity} onChange={(event) => set('minDebtToEquity', event.target.value)} placeholder="不限" /></label>
					<label><span>負債權益比最大（%）</span><input type="number" inputMode="decimal" value={draft.maxDebtToEquity} onChange={(event) => set('maxDebtToEquity', event.target.value)} placeholder="不限" /></label>
					<label><span>流動比率最小（%）</span><input type="number" inputMode="decimal" value={draft.minCurrentRatio} onChange={(event) => set('minCurrentRatio', event.target.value)} placeholder="不限" /></label>

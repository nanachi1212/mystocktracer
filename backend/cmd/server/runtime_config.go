...
// still read in place when they hold settings, portfolio/watchlist databases or agent state.
...
func preferredDataDir(configDirectory string) string {
	...
	for _, candidate := range []string{canonical, filepath.Join(configDirectory, "easy-stock"), filepath.Join(configDirectory, "a-stock-ai")} {
		for _, name := range []string{"settings.json", "taiwan-watchlist.db", "taiwan-portfolio.db"} {
			if isFile(filepath.Join(candidate, name)) {
				return candidate
			}
		}
		if entries, err := os.ReadDir(filepath.Join(candidate, "hermes-home")); err == nil && len(entries) > 0 {
			return candidate
		}
	}
	return canonical
